...
function useMutation(mutation, baseOptions) {
  ...
  var client = (0, _ApolloContext.useApolloClient)(baseOptions.client);

  var _React$useState = _react["default"].useState(getInitialState),
  ...
  var onMutationCompleted = function onMutationCompleted(response, mutationId) {
    ...
    if (errors && errors.length > 0) {
      onMutationError(new _apolloClient.ApolloError({
  ...
  var runMutation = _react["default"].useCallback(function (mutateOptions) {
  ...
  }, [client, mutation, (0, _utils.objToKey)(baseOptions)]);

  return [runMutation, result];
}

function useMutationTracking() {
  var mostRecentMutationId = _react["default"].useRef(0);
